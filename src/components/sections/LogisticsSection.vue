  ...
  <section id="logistica" class="py-24 px-6 sm:px-12 relative overflow-hidden bg-white">
    <div class="max-w-6xl mx-auto">
      ...
      <div class="grid lg:grid-cols-2 lg:items-stretch gap-0 lg:gap-24">
        ...
        >
          <div
            v-for="item in logisticsFeatures" :key="item.title"
        ...
        <div class="relative rounded-t-[2.5rem] lg:rounded-[2rem] overflow-hidden order-1 lg:order-2 h-[440px] sm:h-[480px] lg:h-full lg:min-h-[500px]">
          <div class="absolute inset-x-0 top-0 bottom-0 flex items-start justify-center pointer-events-none px-6">
            <img
              src="@/assets/images/order-deenex.webp"
              alt="Deenex Mobile App"
              class="w-[125%] sm:w-[90%] lg:w-[130%] h-auto object-contain origin-top scale-[1.05] lg:scale-[0.85] pt-12 lg:pt-8"
            />
          </div>
          <div class="absolute inset-x-0 bottom-0 h-[100px] bg-gradient-to-t from-white via-white/70 to-transparent z-20 pointer-events-none"></div>
      ...
      <div class="flex lg:hidden items-center justify-center gap-2 mt-3">
        <button
          v-for="(_, i) in logisticsFeatures"
          :key="i"
