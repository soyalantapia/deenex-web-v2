<template>
  <section id="hero" :class="{ 'hero-ready': ready }" class="hero-section relative flex h-[100svh] min-h-[800px] sm:min-h-[850px] flex-col overflow-hidden pt-24 sm:pt-32 lg:pt-24">
    ...
    <div class="relative z-10 mx-auto flex w-full max-w-[1240px] flex-1 flex-col px-4 text-center sm:px-6 lg:px-8">
      ...
      <!-- Pill Badge (Eyebrow) -->
      <div :class="{ 'animate-rise': ready, 'opacity-0': !ready }" class="mb-8 flex justify-center" style="animation-delay:0.05s">
        <span class="inline-flex items-center gap-2 border border-primary/20 bg-primary/5 text-primary rounded-full px-4 py-1.5 text-xs font-semibold tracking-wide">
          Ventas · Fidelización · Marketing
        </span>
      </div>

      <!-- Headline -->
      <h1 :class="{ 'animate-rise': ready, 'opacity-0': !ready }" class="font-heading text-[clamp(2.4rem,5.8vw,4.2rem)] leading-[1.05] tracking-tighter text-slate-900" style="animation-delay:0.15s">
        <span class="block font-extrabold">Lanzá tu canal propio</span>
        <span class="relative flex items-center justify-center font-extrabold flex-wrap gap-x-[0.25em] sm:gap-x-[0.3em] mt-1">
          <span class="text-slate-900">sin</span>
          <span class="relative block h-[1.3em] overflow-hidden italic text-primary text-left min-w-[200px] sm:min-w-[340px]">
            <div class="wheel-container">
              <div v-for="(word, i) in displayWords" :key="i" class="wheel-word h-[1.3em] flex items-center">{{ word }}.</div>
            </div>
          </span>
        </span>
      </h1>
      ...
      <!-- Subtitle -->
      <p :class="{ 'animate-rise': ready, 'opacity-0': !ready }" class="mx-auto mt-8 max-w-[540px] text-[1.05rem] font-medium leading-[1.7] text-slate-500" style="animation-delay:0.28s">
      ...
      <!-- CTAs -->
      <div :class="{ 'animate-rise': ready, 'opacity-0': !ready }" class="mt-9 mb-5 sm:mb-0 flex items-center justify-center gap-3 sm:gap-4" style="animation-delay:0.4s">
        <a href="#agendar"
          ...
          Comenzar Ahora
          <span class="inline-flex items-center justify-center w-5 h-5 rounded-lg bg-white/15 transition-transform duration-300 group-hover:translate-x-0.5">
        ...
        <a href="#infraestructura"
          ...
          Ver cómo funciona
          <svg width="12" height="12" viewBox="0 0 13 13" fill="none" class="transition-transform duration-300 group-hover:translate-x-0.5">
            <path d="M2 6.5h9M7.5 3l3.5 3.5L7.5 10" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/>
          </svg>
        </a>
      </div>
      ...
      <!-- Social Proof -->
      <div :class="{ 'animate-rise': ready, 'opacity-0': !ready }" class="mt-3 sm:mt-10 flex items-center justify-center gap-3" style="animation-delay:0.5s">
      ...
      <div class="relative flex-1 overflow-visible min-h-[450px]">
        ...
        <!-- Float cards desktop -->
        <div v-for="(pos, i) in floatPositions" :key="i"
             v-show="floatImages[i]"
             :class="['float-card-container hidden sm:block absolute z-20 transition-all duration-1000 ease-out', 
                      pos.class,
        ...
        <!-- Float cards mobile -->
        <div v-if="floatImages[0]" 
             :class="['absolute left-[1%] top-[20%] z-30 block w-[35vw] max-w-[140px] sm:hidden transition-all duration-1000', 
        ...
        <div v-if="floatImages[1]" 
             :class="['absolute right-[2%] top-[35%] z-30 block w-[32vw] max-w-[130px] sm:hidden transition-all duration-1000', 
                      ready ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4']"
        ...
        <!-- MacBook -->
        <div class="absolute inset-x-0 bottom-[30%] z-10 flex justify-center px-4 sm:bottom-[-10%] md:bottom-[-15%] lg:bottom-[-20%]">
          <div class="relative z-10 w-full max-w-[120%] sm:max-w-[100%] lg:max-w-[1300px] xl:max-w-[800px]">
            <!-- Radial glow under mockup -->
            <div class="mockup-glow absolute bottom-[15%] left-1/2 -translate-x-1/2 z-0 h-[200px] w-[80%] max-w-3xl rounded-full bg-primary/20 blur-[80px]"></div>
...
const words = ['comisiones', 'intermediarios', 'dependencia']
...
<style scoped>
.hero-section { background: #f8f9fb; }
...
.orb-1 { width: 600px; height: 600px; left: -8%; top: -5%; animation: orbDrift 20s ease-in-out infinite alternate; }
.orb-2 { width: 700px; height: 700px; right: -12%; top: 20%; animation: orbDrift 25s ease-in-out 3s infinite alternate-reverse; }
.orb-3 { width: 400px; height: 400px; left: 30%; bottom: 10%; animation: orbDrift 18s ease-in-out 5s infinite alternate; }
...
/* Entry animation */
@keyframes rise {
  from { opacity: 0; transform: translateY(24px) scale(0.98); filter: blur(10px); }
  to   { opacity: 1; transform: translateY(0) scale(1); filter: blur(0); }
}
.animate-rise { animation: rise 1s cubic-bezier(0.16, 1, 0.3, 1) both; }

/* MacBook entry */
.mockup-img { opacity: 0; }
.hero-ready .mockup-img { animation: entryMac 1.4s cubic-bezier(0.16, 1, 0.3, 1) 0.5s both; }
@keyframes entryMac {
  from { opacity: 0; transform: translateY(80px) scale(0.92); filter: blur(20px); }
  to   { opacity: 1; transform: translateY(0) scale(1); filter: blur(0); }
}
...
.anim-f1 { animation: floatY 10s ease-in-out infinite; }
.anim-f2 { animation: floatY 9s ease-in-out 1s infinite; }
.anim-f3 { animation: floatY 11s ease-in-out 0.5s infinite; }
.anim-f4 { animation: floatY 9.5s ease-in-out 1.5s infinite; }
