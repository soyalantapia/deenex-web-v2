<template>
  <form @submit.prevent="onSubmit" novalidate>
    ...
    <h1 class="text-[clamp(1.8rem,4vw,2.6rem)] font-extrabold tracking-tighter leading-[1.05] text-slate-900 mb-4">
      Tu plan recomendado.<br />
      <span class="text-primary italic font-light">Pagás solo si te convence después de 15 días.</span>
    </h1>
    <p class="text-base text-slate-500 leading-relaxed mb-8 max-w-md">
      ...
      Podés cambiarlo manualmente más abajo.
    </p>

    <!-- Plan principal: card destacada -->
    <div
      class="relative bg-primary text-white rounded-3xl p-7 mb-6 overflow-hidden"
      v-if="!isEnterprise"
    >
      ...
      <div class="relative">
        <div class="flex items-start justify-between gap-4 mb-5">
          ...
          <span class="bg-emerald-400 text-emerald-950 text-[10px] font-black uppercase tracking-wider px-2.5 py-1 rounded-full">
            15 días gratis
          </span>
        </div>
        ...
        <p class="text-white/80 text-sm leading-relaxed">
          ...
          por venta procesada · sin cargo durante el trial.
        </p>
    ...
    <!-- Savings vs apps de terceros -->
    <div
      v-if="onboarding.savingsVsThirdParty.value > 0 && !isEnterprise"
      class="border border-emerald-200 bg-emerald-50/50 rounded-2xl p-5 mb-6 flex items-start gap-3"
    >
      ...
      <div class="min-w-0">
        <p class="text-sm font-bold text-slate-900 leading-tight">
          Estimás ahorrar
          <span class="text-emerald-600">≈ USD {{ formattedSavings }} / mes</span>
          vs apps de terceros.
        </p>
        <p class="text-xs text-slate-500 mt-1 leading-relaxed">
          Calculado contra una comisión típica del 25% que cobran PedidosYa o Rappi sobre tu GMV.
        </p>
      </div>
    </div>
    ...
    <details class="group mb-2">
      <summary class="list-none cursor-pointer flex items-center justify-between py-3 px-4 rounded-xl hover:bg-slate-50 transition-colors">
        <span class="text-sm font-semibold text-slate-700">¿Querés elegir otro plan?</span>
      ...
      <div class="mt-3 grid grid-cols-2 gap-2.5">
        ...
        >
          <p class="text-sm font-bold text-slate-900">{{ tier.name }}</p>
          <p class="text-xs text-slate-500 mt-0.5">
            <template v-if="tier.monthlyFee !== null">USD {{ tier.monthlyFee }} / mes</template>
    ...
    <p class="mt-6 text-[11px] text-slate-400 leading-relaxed">
      Sin cargo durante 15 días. El cobro recurrente arranca el día 16, cancelás cuando quieras.
    </p>
  </form>
</template>
...
import { TrendingDown, ChevronDown } from 'lucide-vue-next'
...
const formattedSavings = computed(() => {
  return onboarding.savingsVsThirdParty.value.toLocaleString('es-AR')
})
...
function onSubmit() {
  ...
  if (isEnterprise.value) {
    // Para Enterprise saltamos checkout y derivamos al equipo comercial
